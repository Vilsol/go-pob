...
| `m_min\(`                                                               | `min(`                                          |
| `m_floor\(`                                                             | `math.Floor(`                                   |
| `t_insert\(breakdown\[([^]]+)\]\, \"([^"]+)\"\)`                        | `breakdown.AddLine($1, "$2")`                   |
| `t_insert\(breakdown\[([^]]+)\]\, s_format\(\"([^"]+)\",([^)]+)\)\)`    | `breakdown.AddLine($1, fmt.Sprintf("$2",$3))`   |
| `t_insert\(breakdown\.([^]\n]+?)\, \"([^"]+?)\"\)`                      | `breakdown.AddLine("$1", "$2")`                 |
| `t_insert\(breakdown\.([^]\n]+?)\, s_format\(\"([^"]+?)\",([^)]+?)\)\)` | `breakdown.AddLine("$1", fmt.Sprintf("$2",$3))` |
