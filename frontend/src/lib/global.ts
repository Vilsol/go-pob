...
devMode.subscribe((v) => browser && localStorage.setItem('options:devMode', v.toString()));
...
export const markBackendAsLoaded = writable<() => void>();

export const backendLoaded = new Promise<void>((r) => {
  markBackendAsLoaded.set(r);
});
